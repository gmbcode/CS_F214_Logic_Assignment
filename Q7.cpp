...
void collectLiterals(TreeNode* node, std::set<char>& positiveAtoms, std::set<char>& negativeAtoms) {
    if (!node) return;
...
bool isSatisfiableClause(TreeNode* clause) {
    std::set<char> positiveAtoms;
    std::set<char> negativeAtoms;
...
pair<int, int> isCNF(TreeNode* root) {
    ...
    return make_pair(validNum, invalidNum);
}